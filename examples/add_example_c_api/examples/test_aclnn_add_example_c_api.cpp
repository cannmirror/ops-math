...
#include <algorithm>
#include "acl/acl.h"
#include "aclnn_add_example_c_api.h"
                    ...
                    const std::vector<float>& selfXHostData, const std::vector<float>& selfYHostData)
{
    ...
    for (int64_t i = 0; i < PRINT_LENGTH; i++) {
        LOG_PRINT("add_example_c_api first input[%ld] is: %f, second input[%ld] is: %f, result[%ld] is: %f\n", i, selfXHostData[i], i, selfYHostData[i], i, resultData[i]);
    }
}
...
int main()
{
    ...
    ret = aclnnAddExampleCApiGetWorkspaceSize(selfX, selfY, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAddExampleCApiGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    ret = aclnnAddExampleCApi(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAddExampleCApi failed. ERROR: %d\n", ret); return ret);
