...
#include "nn_other.h"
#include "../op_graph/tanh_grad_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape, inputValues)                                        \
    do {                                                                                                             \
        ...
        tanh_grad1.set_input_##inputName(placeholder##inputIndex);                                                   \
...
{
    Status ret = SUCCESS;
    auto tanh_grad1 = op::TanhGrad("tanh_grad1");
    tanh_grad1.set_attr_complex_conj(false);
    std::vector<int64_t> yShape = {2, 3};
    std::vector<double> yData = {0.2, 0.5, -0.5, 0.25, -0.25, 0.75};
    ADD_INPUT(1, y, inDtype, yShape, yData);
    std::vector<int64_t> dyShape = {2, 3};
    std::vector<double> dyData = {1.0, 1.0, 1.0, 2.0, -2.0, 0.5};
    ADD_INPUT(2, dy, inDtype, dyShape, dyData);

    outputs.push_back(tanh_grad1);
    return SUCCESS;
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < input_num; i++) {
        string input_file = "./tc_ge_irrun_test_tanh_grad_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        string output_file = "./tc_ge_irrun_test_tanh_grad_npu_output_" + std::to_string(i) + ".bin";
